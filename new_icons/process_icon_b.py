#!/usr/bin/env python3
"""Process icon_b: simplify to 2 colors (dark gray + transparent)."""

from PIL import Image

def process_icon_b(img):
    """Keep only dark gray foreground, make background transparent."""
    # Convert to RGBA
    ...
    # Define target foreground color (dark gray)
    DARK_GRAY = (55, 65, 75)  # A nice dark charcoal gray
    print(f"Target foreground color: RGB{DARK_GRAY}")
    ...
    for y in range(height):
        for x in range(width):
            ...
            if bg_diff < 80:  # tolerance for background
                ...
                # It's foreground - snap to dark gray
                pixels[x, y] = (*DARK_GRAY, 255)
                foreground_count += 1

    print(f"\nProcessed {total_pixels} pixels:")
    print(f"  Foreground (dark gray): {foreground_count} ({100*foreground_count/total_pixels:.1f}%)")
...
def main():
    # Load icon_b
    input_path = "new_icons/icon_b.jpg"
